...
const getProducts = asyncHandler(async (req, res) => {
  ...
  if (req.query.subcategory)
    filter.subcategory = req.query.subcategory === "true";
  ...
  if (req.query.isDigital !== undefined)
    filter.isDigital = req.query.isDigital === "true";
...
const createProduct = asyncHandler(async (req, res) => {
  ...
  if (!parentSubategory || !parentSubategory.isActive) {
    return res.status(400).json({
      success: false,
      message: "La subcategoria especifica no existe no esta activa",
    });
  }

  console.log(parentSubategory.category.toString(), category);
...
const toggleProductStatus = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    success: true,
    message: `Product ${
      product.isActive ? "activada" : "desactivada"
    } correctamente`,
    data: product,
  });
});
